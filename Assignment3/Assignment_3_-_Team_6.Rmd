...
## 1st Visualization: PSF of Private Properties by Loaction
```{r}
...
# plot the bar chart 
ggplot(psf.pp.df) + geom_bar(aes(x=District, y= PSF),stat="identity", fill="lightpink") + coord_flip()  + labs(x="", y="PSF") + ggtitle("Average PSF of Private Properties by Location") + theme(axis.text.y=element_text(size = rel(0.8))) + theme_minimal() 
```

## 2nd Visualization: PSF of HDB by Loaction
```{r}
...
# plot the bar chart 
ggplot(psf.hdb.df) + geom_bar(aes(x=HDB.Town, y= PSF),stat="identity", fill="lightblue") + coord_flip()  + labs(x="", y="PSF") + ggtitle("Average PSF of HDB by Location") + theme(axis.text.y=element_text(size = rel(0.8))) + theme_minimal() 
```
